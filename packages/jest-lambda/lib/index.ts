...
const createRunner = (runPath: string, { getExtraOptions }: any = {}) => {
        ...
        (promise: any, test: any) =>
          mutex(() =>
            promise
              ...
              .then((result: any) => onResult(test, result))
              .catch((err: any) => onFailure(test, err)),
          ),
        Promise.resolve(),
      );
    }
      ...
      const runTestInWorker = (test: any) =>
        mutex(() => {
          ...
          return onStart(test).then(() => {
            ...
            return (worker as any).default(baseOptions);
          });
        });
